    ...
    <body style="margin:0;background-image: url('<?php
        ...
        else
        {
            if(isset($_POST['revBtn']))
            {
                ...
                if(mysqli_affected_rows($con)>0)
                { 
                    mysqli_query($con,"update rnr set review_head= '".mysqli_real_escape_string($con,$_POST['heading'])."', review='".mysqli_real_escape_string($con,$_POST['review'])."', timestamp=NOW() where m_id=".$mid." and uemail='".$_SESSION['email']."';");
                }
                else
                { 
                    mysqli_query($con,"insert into rnr (m_id,uemail,review_head,review,timestamp) values(".$mid.",'".$_SESSION['email']."','".mysqli_real_escape_string($con,$_POST['heading'])."','".mysqli_real_escape_string($con,$_POST['review'])."',NOW());");
                } 
            }
